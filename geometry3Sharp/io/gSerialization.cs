...
namespace g3
{
	public static class gSerialization
	{
		...
		public static void Restore(out IParametricCurve2d curve, BinaryReader reader)
		{
			...
			{
				var circle = new Circle2d(Vector2d.Zero, 1.0);
				Restore(ref circle, reader);
				curve = circle;
			}
			else if (nType == 3)
			{
				var arc = new Arc2d(Vector2d.Zero, 1.0, 0, 1);
		...
		public static void Restore(DMesh3 mesh, BinaryReader reader)
		{
			...
			var components = (MeshComponents)reader.ReadInt32();
		...
		public static SimpleStore Restore(string sPath)
		{
			...
			using (var stream = new FileStream(sPath, FileMode.Open))
			{
				using (var r = new BinaryReader(stream))
				{
					...
					{
						var m = new DMesh3(); gSerialization.Restore(m, r); s.Meshes.Add(m);
					}
					...
					{
						var l = new List<int>(); gSerialization.Restore(l, r); s.IntLists.Add(l);
					}
				}
			}
			return s;
