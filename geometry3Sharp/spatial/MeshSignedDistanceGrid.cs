...
namespace g3
{
	...
	public class MeshSignedDistanceGrid
	{
		...
		private void check_neighbour(DenseGrid3f phi, DenseGrid3i closest_tri,
		...
		private void compute_signs(int ni, int nj, int nk, DenseGrid3f distances, DenseGrid3i intersection_counts)
		{
			...
			if (UseParallel)
			{
				...
				var box = new AxisAlignedBox2i(0, 0, nj, nk);
		...
		private void make_level_set3(Vector3f origin, float dx,
							 ...
							 DenseGrid3f distances, int exact_band)
		{
			...
			var closest_tri = new DenseGrid3i(ni, nj, nk, -1);

			// intersection_count(i,j,k) is # of tri intersections in (i-1,i]x{j}x{k}
			var intersection_count = new DenseGrid3i(ni, nj, nk, 0);
			...
			foreach (int tid in Mesh.TriangleIndices())
			{
				...
				for (int k = k0; k <= k1; ++k)
				{
					for (int j = j0; j <= j1; ++j)
					{
						for (int i = i0; i <= i1; ++i)
						{
							var gx = new Vector3d((float)i * dx + origin[0], (float)j * dx + origin[1], (float)k * dx + origin[2]);
							float d = (float)point_triangle_distance(ref gx, ref xp, ref xq, ref xr);
		...
		private void make_level_set3_parallel(Vector3f origin, float dx,
							 int ni, int nj, int nk,
							 DenseGrid3f distances, int exact_band)
		{
			distances.resize(ni, nj, nk);
					...
					for (int j = j0; j <= j1; ++j)
					{
						...
						for (int i = i0; i <= i1; ++i)
						{
							...
							if (d < distances[i, j, k])
							{
								...
								if (d < distances[i, j, k])
								{    // have to check again in case grid changed in another thread...
									distances[i, j, k] = d;
									closest_tri[i, j, k] = tid;
								}
		...
		private void make_level_set3_parallel_floodfill(Vector3f origin, float dx,
							 int ni, int nj, int nk,
							 DenseGrid3f distances, int exact_band)
		{
			distances.resize(ni, nj, nk);
		...
		private void make_level_set3_parallel_spatial(Vector3f origin, float dx,
							 int ni, int nj, int nk,
							 DenseGrid3f distances, int exact_band)
		{
			distances.resize(ni, nj, nk);
			...
			gParallel.ForEach(grid.Indices(), (idx) =>
			{
				if (distances[idx] == 1)
				{
					...
					var p = new Vector3d((float)i * dx + origin[0], (float)j * dx + origin[1], (float)k * dx + origin[2]);
		...
		// single sweep pass
		private void sweep(DenseGrid3f phi, DenseGrid3i closest_tri,
				   ...
				   int di, int dj, int dk)
		{
			...
			for (int k = k0; k != k1; k += dk)
			{
				...
				for (int j = j0; j != j1; j += dj)
				{
					...
					{
						var gx = new Vector3d(i * dx + origin[0], j * dx + origin[1], k * dx + origin[2]);
		...
		private void sweep_pass(Vector3f origin, float dx,
