	...
	public class Polygon2dBoxTree
	{
		...
		void build_sequential(Polygon2d poly)
		{
			...
			for (int si = 0; si < NV; si += 2)
			{
				...
				var seg1 = new Segment2d(poly[si], v1);
				var box = new Box2d(seg1);
				if (si < NV - 1)
				{
					var seg2 = new Segment2d(v1, poly[(si + 2) % NV]);
					var box2 = new Box2d(seg2);
			...
			while (done == false)
			{
				...
				for (int k = 0; k < N; k += 2)
				{
					var mbox = Box2d.Merge(ref boxes[prev_layer_start + k], ref boxes[prev_layer_start + k + 1]);
					boxes[bi++] = mbox;
				}
