	...
	public class MeshEditor
	{
		...
		public virtual int[] AddTriangleFan_OrderedVertexLoop(int center, int[] vertex_loop, int group_id = -1)
		{
			...
			for (i = 0; i < N; ++i)
			{
				...
				var newT = new Index3i(center, b, a);
		...
		{
			var in_set = new HashSet<int>(triangles);
		...
		public void ReverseTriangles(IEnumerable<int> triangles, bool bFlipVtxNormals = true)
		{
			...
			{
				var donev = new BitArray(Mesh.MaxVertexID);
		...
		{
			var bowties = new List<int>(MeshIterators.BowtieVertices(Mesh));
			int iter = 0;
			...
			DuplicateTriBehavior eDuplicateBehavior = DuplicateTriBehavior.AssertAbort)
		{
			...
			var done_v = new IndexFlagSet(submesh.MaxVertexID, submesh.TriangleCount / 2);
		...
		{
			var editor = new MeshEditor(appendTo);
		...
		{
			var C = new MeshConnectedComponents(Mesh);
			C.FindConnectedT();
			if (C.Count == 1)
			{
				return 0;
