	...
	public partial class DMesh3 : IDeformableMesh
	{
		...
		public CompactInfo CompactCopy(DMesh3 copy, bool bNormals = true, bool bColors = true, bool bUVs = true)
		{
			if (copy.IsCompact)
			{
				...
				var ci = new CompactInfo() { MapV = new IdentityIndexMap() };
				return ci;
			}
		...
		public MeshComponents Components
		{
			get
			{
				...
				if (normals != null)
				{
					c |= MeshComponents.VertexNormals;
				}

				if (colors != null)
				{
					c |= MeshComponents.VertexColors;
				}

				if (uv != null)
				{
					c |= MeshComponents.VertexUVs;
				}

				if (triangle_groups != null)
				{
					c |= MeshComponents.FaceGroups;
				}
		...
		public Frame3f GetVertexFrame(int vID, bool bFrameNormalY = false)
		{
			...
			int vi = 3 * vID;
			var v = new Vector3d(vertices[vi], vertices[vi + 1], vertices[vi + 2]);
			var normal = new Vector3d(normals[vi], normals[vi + 1], normals[vi + 2]);
			int eid = vertex_edges.First(vID);
			int ovi = 3 * edge_other_v(eid, vID);
			var ov = new Vector3d(vertices[ovi], vertices[ovi + 1], vertices[ovi + 2]);
			Vector3d edge = (ov - v);
			edge.Normalize();
		...
		public Frame3f GetTriFrame(int tID, int nEdge = 0)
		{
			...
			var v1 = new Vector3d(vertices[a], vertices[a + 1], vertices[a + 2]);
			var v2 = new Vector3d(vertices[b], vertices[b + 1], vertices[b + 2]);
			var v3 = new Vector3d(vertices[c], vertices[c + 1], vertices[c + 2]);
		...
		public double GetTriSolidAngle(int tID, ref Vector3d p)
		{
			...
			var a = new Vector3d(vertices[ta] - p.x, vertices[ta + 1] - p.y, vertices[ta + 2] - p.z);
			int tb = 3 * triangles[ti + 1];
			var b = new Vector3d(vertices[tb] - p.x, vertices[tb + 1] - p.y, vertices[tb + 2] - p.z);
			int tc = 3 * triangles[ti + 2];
			var c = new Vector3d(vertices[tc] - p.x, vertices[tc + 1] - p.y, vertices[tc + 2] - p.z);
		...
		public double GetTriInternalAngleR(int tID, int i)
		{
			int ti = 3 * tID;
			int ta = 3 * triangles[ti];
			var a = new Vector3d(vertices[ta], vertices[ta + 1], vertices[ta + 2]);
			int tb = 3 * triangles[ti + 1];
			var b = new Vector3d(vertices[tb], vertices[tb + 1], vertices[tb + 2]);
			int tc = 3 * triangles[ti + 2];
			var c = new Vector3d(vertices[tc], vertices[tc + 1], vertices[tc + 2]);
			if (i == 0)
			{
		...
		public Index2i GetOrientedBoundaryEdgeV(int eID)
		{
			if (edges_refcount.isValid(eID))
			{
				...
				if (edges[ei + 3] == InvalidID)
				{
					...
					int ti = 3 * edges[ei + 2];
					var tri = new Index3i(triangles[ti], triangles[ti + 1], triangles[ti + 2]);
		...
		public bool IsGroupJunctionVertex(int vID)
		{
			...
			foreach (int eID in vertex_edges.ValueItr(vID))
			{
				var et = new Index2i(edges[4 * eID + 2], edges[4 * eID + 3]);
				for (int k = 0; k < 2; ++k)
				{
					if (et[k] == InvalidID)
					{
		...
		public bool IsBowtieVertex(int vID)
		{
			if (vertices_refcount.isValid(vID))
			{
				...
				{
					int i = 3 * prev_tid;
					var tv = new Index3i(triangles[i], triangles[i + 1], triangles[i + 2]);
					var te = new Index3i(triangle_edges[i], triangle_edges[i + 1], triangle_edges[i + 2]);
