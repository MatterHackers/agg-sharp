...
namespace g3
{
	public static class MeshConstraintUtil
	{
		...
		{
			var edgeCons = new EdgeConstraint(EdgeRefineFlags.NoFlip | EdgeRefineFlags.NoSplit);
			var vertCons = new VertexConstraint(true, setID);
		...
		{
			var edgeCons = new EdgeConstraint(EdgeRefineFlags.NoFlip | EdgeRefineFlags.NoCollapse);
			var vertCons = new VertexConstraint(true, setID);
		...
		{
			var vc = new VertexConstraint(target);
			int N = loopV.Count;
			for (int i = 0; i < N; ++i)
			{
			...
			var ec = new EdgeConstraint(EdgeRefineFlags.NoFlip, target);
		...
		public static void PreserveBoundaryLoops(MeshConstraints cons, DMesh3 mesh)
		{
			...
			foreach (EdgeLoop loop in loops)
			{
				...
				var target = new DCurveProjectionTarget(loopC);
