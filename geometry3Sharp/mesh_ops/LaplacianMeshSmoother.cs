...
namespace g3
{
	public class LaplacianMeshSmoother
	{
		...
		public bool SolveMultipleCG(Vector3d[] Result)
		{
			...
			var SolverX = new SparseSymmetricCG()
			{
				B = Bx,
				X = Sx,
			...
			var SolverY = new SparseSymmetricCG()
			{
				B = By,
				X = Sy,
				MultiplyF = CombinedMultiply,
			...
			var SolverZ = new SparseSymmetricCG()
			{
				B = Bz,
				X = Sz,
				MultiplyF = CombinedMultiply,
			...
			var solvers = new SparseSymmetricCG[3] { SolverX, SolverY, SolverZ };
			bool[] ok = new bool[3];
		...
		public bool SolveMultipleRHS(Vector3d[] Result)
		{
			...
			var Solver = new SparseSymmetricCGMultipleRHS()
			{
				...
				X = X,
				MultiplyF = CombinedMultiply,
				PreconditionMultiplyF = CombinedPreconditionerMultiply,
				UseXAsInitialGuess = true
			};
			...
			double borderWeight = 10.0, double interiorWeight = 0.0)
		{
			...
			var region = new RegionOperator(mesh, triangles);
			DSubmesh3 submesh = region.Region;
			DMesh3 smoothMesh = submesh.SubMesh;
			var smoother = new LaplacianMeshSmoother(smoothMesh);

			// map fixed verts to submesh
			var subFixedVerts = new HashSet<int>();
			...
			if (constrained.Count > 0)
			{
				...
				{
					var next_layer = new HashSet<int>();
