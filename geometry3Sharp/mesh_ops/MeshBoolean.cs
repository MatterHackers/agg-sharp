...
namespace g3
{
	public class MeshBoolean
	{
		...
		void resolve_vtx_pairs()
		{
			...
			var targetVerts = new HashSet<int>(MeshIterators.BoundaryVertices(cutTargetMesh));
			var toolVerts = new HashSet<int>(MeshIterators.BoundaryVertices(cutToolMesh));
		...
		int find_nearest_edge(DMesh3 mesh, Vector3d v, HashSet<int> vertices)
		{
			...
			foreach (int eid in mesh.BoundaryEdgeIndices())
			{
				...
				var seg = new Segment3d(mesh.GetVertex(ev.a), mesh.GetVertex(ev.b));
