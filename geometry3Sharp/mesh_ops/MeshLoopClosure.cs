...
namespace g3
{
	public class MeshLoopClosure
	{
		...
		public void Close_Flat()
		{
			...
			var extrude = new MeshExtrudeLoop(Mesh, fill_loop);
			...
			var loop_smooth = new MeshLoopSmooth(Mesh, extrude.NewLoop);
			...
			// fill result
			var filler = new SimpleHoleFiller(Mesh, extrude.NewLoop);
			...
			spatial.Build();
			var target = new MeshProjectionTarget(Mesh, spatial);
		...
		public static void smooth_loop(DMesh3 mesh, EdgeLoop loop, int nRings)
		{
			...
			var loop_smooth = new MeshLoopSmooth(mesh, loop);
