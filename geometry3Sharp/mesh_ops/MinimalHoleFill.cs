...
namespace gs
{
	...
	public class MinimalHoleFill
	{
		...
		{
			// do a simple fill
			var simplefill = new SimpleHoleFiller(Mesh, FillLoop);
			...
			var intial_fill_tris = new HashSet<int>(simplefill.NewTriangles);
			...
			var remesh1 = new RemesherPro(fillmesh);
			...
			var remaining_edges = new HashSet<int>(fillmesh.EdgeIndices());
			var updated_edges = new HashSet<int>();
		...
		{
			var interiorv = new HashSet<int>(MeshIterators.InteriorVertices(fillmesh));
			int prev_count = 0;
