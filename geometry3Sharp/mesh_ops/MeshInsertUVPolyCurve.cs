...
namespace g3
{
	...
	public class MeshInsertUVPolyCurve
	{
		...
		{
			var query = new PrimalQuery2d(PointF);
			...
			if (UseTriSpatial)
			{
				...
				var bounds2 = new AxisAlignedBox2d(bounds3.Min.xy, bounds3.Max.xy);
			...
			double bary_tol, double spatial_tol, out bool is_existing_v)
		{
			...
			if (split_edge >= 0)
			{
				...
				Index2i ev = Mesh.GetEdgeV(eid);
				var seg = new Segment2d(PointF(ev.a), PointF(ev.b));
		...
		{
			var OnCurveVerts = new HashSet<int>();     // original vertices that were epsilon-coincident w/ curve vertices
			...
			var ZeroEdges = new HashSet<int>();
			var ZeroVertices = new HashSet<int>();
			OnCutEdges = new HashSet<int>();

			var NewEdges = new HashSet<int>();
			var NewCutVertices = new HashSet<int>();
			sbyte[] signs = new sbyte[2 * Mesh.MaxVertexID + 2 * Curve.VertexCount];

			var segTriangles = new HashSet<int>();
			var segVertices = new HashSet<int>();
			var segEdges = new HashSet<int>();
			...
			for (int si = 0; si < N; ++si)
			{
				...
				var seg = new Segment2d(Curve[i0], Curve[i1]);
				...
				if (triSpatial != null)
				{
					...
					var segBounds = new AxisAlignedBox2d(seg.P0); segBounds.Contain(seg.P1);
				...
				foreach (int eid in edges)
				{
					...
					var edge_seg = new Segment2d(va, vb);
					var intr = new IntrSegment2Segment2(seg, edge_seg);
					intr.Compute();
		...
		{
			var curve_verts = new HashSet<int>(CurveVertices);

			var remaining_edges = new List<int>();
