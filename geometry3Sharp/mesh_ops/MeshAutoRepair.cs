...
namespace gs
{
	...
	public class MeshAutoRepair
	{
		...
		public bool Apply()
		{
			...
			ProgressReporter?.Invoke(0.0, "Starting mesh repair");
		...
		all_done:
             ...
             */
			ProgressReporter?.Invoke(0.98, "Compacting mesh");
			Mesh = new DMesh3(Mesh, true);
			MeshNormals.QuickCompute(Mesh);

			ProgressReporter?.Invoke(1.0, "Repair complete");
			return true;
		}
		...
		bool remove_duplicate_faces(double vtxTolerance, out int nRemoved)
		{
			...
			try
			{
				var dupe = new RemoveDuplicateTriangles(Mesh);
