...
namespace g3
{
	public class MeshLocalParam
	{
		...
		{
			pos -= f.Origin;
			var uv = new Vector2f(pos.Dot(f.X), pos.Dot(f.Y));
			return uv;
		}
		...
		Vector2f propagate_uv(Vector3f pos, Vector2f nbrUV, ref Frame3f fNbr, ref Frame3f fSeed)
		{
			...
			var mFrameRotate = new Matrix2f(fCosTheta, fSinTheta, -fSinTheta, fCosTheta);
		...
		void update_uv_expmap(GraphNode node)
		{
			...
			var parentFrame = new Frame3f(parentPos, NormalF(parent_id));
		...
		void update_uv_upwind_expmap(GraphNode node)
		{
			...
			foreach (var nbr_id in NeighboursF(node.id))
			{
				...
				if (nbr_node.frozen)
				{
					...
					var nbr_frame = new Frame3f(nbr_pos, NormalF(nbr_id));
